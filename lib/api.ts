...
import type { Client360Data } from '../cfo-console/app/console/Client360';
...
export const saveFinancials = (msmeId: string, body: Row) =>
    post(`/msme/${msmeId}/financials`, body); // returns { ok, score }
...
export interface ExtractedStatement {
    parsed: boolean; method: string; confidence: 'high' | 'medium' | 'low';
    closing_balance: number | null; opening_balance: number | null;
    total_inflow: number | null; total_outflow: number | null;
    avg_daily_outflow: number | null; account_type: string | null;
    accounts_count: number | null; period_from: string | null;
    period_to: string | null; days: number | null;
}
...
export const uploadDocument = (msmeId: string, file: File, docType: string): Promise<UploadResult> => {
...
export interface DocRow {
    id: string; doc_type: string; file_name: string; status: string;
    extracted: ExtractedStatement | null; period_from: string | null;
    period_to: string | null; size_bytes: number | null; created_at: string;
}
