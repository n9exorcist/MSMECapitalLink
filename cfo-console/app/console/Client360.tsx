...
export default function Client360({ data = SRI_SAI }: { data?: Client360Data }) {
  const d = data;
  return (
    <div className="c360" style={rootVars}>
      {/* TOP BAR */}
      ...
      <div className="shell">
        {/* LEFT RAIL */}
        <nav className="rail">
          <div className="grp eyebrow">Client views</div>
          {[['Overview', '', true], ['Financials', '3-yr'], ['Banking', '!'], ['GST', '24'],
          ['Compliance', ''], ['Receivables / Payables', ''], ['Loans', ''], ['Green Profile', ''], ['Documents', '']
          ].map(([label, badge, active]) => (
            <div key={label as string} className={`navitem${active ? ' active' : ''}`}>
              <span><span className="dot" />&nbsp; {label}</span>
              {badge ? <span className="badge">{badge}</span> : null}
            </div>
          ))}
          <div className="grp eyebrow" style={{ marginTop: 6 }}>Banker tools</div>
          {['Ratio Library', 'WC Eligibility (MPBF)', 'Term-Loan Eligibility', 'Migration Pathway', 'Green Finance'].map((t) => (
            <div key={t} className="navitem"><span><span className="dot" />&nbsp; {t}</span></div>
          ))}
        </nav>
      ...
      <style jsx>{`
        ...
        .c360{font-family:"Segoe UI",-apple-system,system-ui,Roboto,Arial,sans-serif;color:var(--text);
          ...
          background:radial-gradient(1100px 480px at 100% -8%,rgba(15,118,110,.08),transparent 55%),var(--bg)}
        .num{font-variant-numeric:tabular-nums;letter-spacing:-.01em}
        .eyebrow{font-size:10.5px;font-weight:800;letter-spacing:.09em;text-transform:uppercase;color:var(--muted)}
        .topbar{background:linear-gradient(135deg,var(--navy) 0%,var(--navy2) 62%,#0d3354 100%);color:#fff;
          padding:10px 18px;display:flex;align-items:center;gap:18px;box-shadow:0 6px 22px -10px rgba(11,46,79,.5);position:sticky;top:0;z-index:5}
        ...
        .schip{background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.16);border-radius:11px;padding:6px 12px;min-width:104px;text-align:right;box-shadow:inset 0 1px 0 rgba(255,255,255,.18)}
        ...
        .shell{display:grid;grid-template-columns:212px 1fr 286px;gap:12px;padding:12px 14px;max-width:1560px;margin:0 auto;align-items:start}
        .rail,.aside{background:var(--surface);border:1px solid var(--border);border-radius:12px;box-shadow:0 1px 2px rgba(15,23,42,.04),0 10px 28px -20px rgba(11,46,79,.25)}
        ...
        .panel{background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:12px;box-shadow:0 1px 2px rgba(15,23,42,.04),0 10px 28px -20px rgba(11,46,79,.25)}
        ...
        .bar .col{width:100%;border-radius:6px 6px 0 0;background:linear-gradient(180deg,#2E6FA6,#0B2E4F)}
        .bar.peak .col{background:linear-gradient(180deg,#14B8A6,#0F766E)}
        ...
        /* ≤1080: collapse shell, STACK the top bar, rail becomes a scroll strip */
        @media (max-width:1080px){
          ...
          .schip{flex:1;min-width:0;text-align:left;padding:6px 10px}
          .toolbar{order:3;flex:1 1 100%;flex-direction:row;align-items:center;justify-content:space-between;min-width:0}
        }
        /* ≤760: stack score tiles, the WC/turnover pair, and doc tiles */
