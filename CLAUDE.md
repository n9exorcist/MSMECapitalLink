...
- **Auth is mode-switched (`lib/authMode.ts`, `EXPO_PUBLIC_AUTH_MODE`).** Default `mock`: `login.tsx` simulates OTP and stores a literal `'mock-jwt-token-123'`; role hardcoded `'owner'` — don't assume the stored token is a valid Supabase JWT. Set `EXPO_PUBLIC_AUTH_MODE=supabase` for real phone OTP (`supabase.auth.signInWithOtp`/`verifyOtp`), which needs an SMS provider configured in Supabase Auth → Phone; then the session is supabase-js's (AsyncStorage) and role comes from user metadata. `useAuthStore.checkAuth()` runs on boot (from `app/_layout.tsx`) to restore the session; `index.tsx` holds on a `hydrated` flag until it completes.
